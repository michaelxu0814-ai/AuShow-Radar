...
- **2026-09-21** | 032号 | 单场演出安利（Rolling Donkey 中文喜剧开放麦 · 悉尼每周二）| fact-audit 12 条全 GREEN，RED=0。角度为"周更的确定性"——把看华语演出从"一年等一次"变成"每周二的一个固定小节目"，与 006(为什么去)/011(在哪看)/022(台上发生什么)零重叠。信源：events.json verified 条目 + Eventbrite 售票页(WebFetch 复核：每周二 7:30 PM / Chippo Hotel 87-91 Abercrombie St / Multiple dates / 未标价)。4 张卡，HTML 由 cards/022 复制仅换文案(主题 token/CSS/字体未动，已 diff 确认第 1–845 行与 001 逐字相同)。渲染与 Telegram 推送交由 run_daily_xhs.sh。⚠️ PORTFOLIO.md 最后评审 2026-09-06，今日 09-21 已 15 天，超出 14 天节律，建议尽快跑 /portfolio-review。
- **2026-09-22** | 033号 | 转票防骗指南（黄牛加价红线）| fact-audit 7 条全 GREEN，RED=0。角度为"加价超10%即违规"——把"报价明显高于官方票价"拎出来当黄牛识别信号，与九篇前作（付款前/假网站/被骗后/卖家/账号/免费中奖/付款后拉黑前/优惠票资格/假退款）零重叠。信源：NSW Fair Trading ticket-reselling 页(WebFetch 抓取原文：转卖价≤原价+最多10%交易成本、广告须列明原价+座位、最高罚$110K/$22K、违规票主办方可取消、Fair Trading 无权责令退款)+维州 Major Events Act 2009(declared major events 转卖价≤票面+10%，djsir/austlii 当日 403 取 WebSearch 抽取，维州罚金数字因各信源口径不一且页面 403 已主动剔除)。4 张卡，HTML 由 cards/001 复制仅换文案(主题 token/CSS/字体逐字保留，head 第 1–845 行与 001 仅 `<title>` 一行差异)。渲染与 Telegram 推送交由 run_daily_xhs.sh。⚠️ PORTFOLIO.md 最后评审 2026-09-06，今日 09-22 已 16 天，超出 14 天节律，建议尽快跑 /portfolio-review。
- **2026-09-23** | 034号 | 场馆攻略（Sydney Event Centre / The Star · 悉尼 Pyrmont）| fact-audit 10 条全 GREEN，RED=0。场馆取 verified 条目「袁娅维 TIA RAY「月亮撒野」世界巡演 悉尼站」(08-22，已过期) 的 Sydney Event Centre (The Star)——verified 池里最后一座未写过的场馆(前 7 座：Marvel/ENGIE/Chippo/Margaret Court/TikTok Ent Centre/Club Voltaire/Palais)。角度为"场馆开在赌场综合体里"：非临街独栋、Level 3 顶楼、赌场游戏区 18+（Star 官方 T&C "Guests must be aged 18 years or over to enter the casino"）、官方票务 Ticketmaster。信源：star.com.au 官方 PDF（80 Pyrmont St 地址 + 赌场 18+）、mice.net.au + cimbusinessevents.com.au（Foundation Theatres 2025-09 接手 + Ticketmaster 票务伙伴）、OnlySydney + engageHQ（Level 3 顶楼）、Moovit（轻轨 L1 The Star 站 / 渡轮 F4 Pyrmont Bay / 公交 389·443·501）。主动剔除：场馆自身是否 18+（官方场馆页 JS 渲染仅返回空壳；"The Star Entertainment Theatre" 的 Conditions of Entry PDF 名称无法与 Sydney Event Centre 对应，不引用）、容量/座位数（各来源 1400/1530/2200/3000/1550 出入极大）、Box office 位置（第三方所指为 Sydney Lyric）、停车价格/班次时刻、"进馆是否经过赌场游戏区"（唯一来源 2019 年第三方 handout 超时且过时）。4 张卡，HTML 由 cards/030 复制仅换文案（主题 token/CSS/字体逐字保留，仅 `<title>` 与 4 个 poster 区块文案差异）。渲染与 Telegram 推送交由 run_daily_xhs.sh。⚠️ PORTFOLIO.md 最后评审 2026-09-06，今日 09-23 已 17 天，超出 14 天节律，建议尽快跑 /portfolio-review。上游遗留未解除项（本次未新增）：AKMU 两场 events.json 字段自 08-24 挂 OPEN 未修、Loadingzone "常驻开放麦/on_sale" 自 09-14 无法证实——三者均已过/待修，不影响场馆攻略线（场馆攻略只要求场馆出现在 verified 演出里）。
- **2026-09-24** | 035号 | 本周开票汇总（官方购票平台清单）| fact-audit 27 条全 GREEN，RED=0。角度为第 6 种切分"按官方购票平台分"（ticket_platform 首次当组织原则，005=时间线/009=提前量/014=城市/021=票价档位/028=倒计时），把 3 场按 Eventbrite(1)/Ticketmaster(2) 两档列，落点接账号核心"官方渠道是唯一标准"。信源：events.json 三条 verified 条目逐字字段 + 今日 WebSearch 复核（Marvel Stadium 官方活动页、sopa.nsw.gov.au、Eventbrite Rolling Donkey 页、Ticketmaster AU 艺人页）全部一致；墨站 Marvel 官方页标 7:00pm 疑为开门时间，采信 events.json 的 19:30（Ticketmaster/festivaly/澳洲微报同口径）。3 张卡，HTML 由 cards/028 复制仅换文案（主题 token/CSS/字体逐字保留，仅 `<title>` 与 3 个 poster 区块文案差异）。渲染与 Telegram 推送交由 run_daily_xhs.sh。⚠️ PORTFOLIO.md 最后评审 2026-09-06，今日 09-24 已 18 天，超出 14 天节律，建议尽快跑 /portfolio-review。上游遗留未解除项（本次未新增）：AKMU 两场 events.json 字段自 08-24 挂 OPEN 未修（且演出日已过）、Loadingzone "常驻开放麦/on_sale" 自 09-14 无法证实——均不影响汇总线（已被"已过期/无法核实"理由剔除）。
- **2026-09-25** | 036号 | 单场演出安利（周杰伦「粉色 墨尔本 嘉年华Ⅱ」· 全澳首站）| fact-audit 15 条全 GREEN，RED=0。角度为"全澳首站"——这轮澳洲只有墨尔本 10-17 / 悉尼 11-21 两站、墨尔本先开演，把"全澳第一批看到"当卖点，与 001(官宣)/004(场馆)/015(倒计时+限购/价位)/018(名字来历)/025(上次 vs 这次)/021(票价档位)/028(剩余时间)/035(平台)零重叠。信源：events.json verified 条目逐字字段 + 今日 WebSearch 复核（ausmusicscene "Carnival II Australian Tour 2026" 仅列墨/悉两场、Marvel Stadium 官方页 "Saturday 17 October 2026"、Ticketmaster AU 艺人页现标 Low Availability——Low Availability 仅正文引用一次不进卡片，同 018 口径）。4 张卡，HTML 由 cards/001 复制仅换文案（主题 token/CSS/字体逐字保留，仅 `<title>` 与 P1/P3/P4 文案差异，P2 票根与 001 完全相同未动）。渲染与 Telegram 推送交由 run_daily_xhs.sh。⚠️ PORTFOLIO.md 最后评审 2026-09-06，今日 09-25 已 19 天，超出 14 天节律，建议尽快跑 /portfolio-review。上游遗留未解除项（本次未新增）：AKMU 两场 events.json 字段自 08-24 挂 OPEN 未修（且演出日 09-18/09-20 均已过）、Loadingzone "常驻开放麦/on_sale" 自 09-14 无法证实（今日 WebSearch 复核仍无明确 2026 开放麦场次）——均不影响单场安利线（已被"过期/无法核实"理由跳过，改用最久未安利的可讲条目重讲）。
